...
author: "JAFS Central: Ali Al Kenani, Jessie Preston, Wan Sze Mun and Freya Maw"
date: "`r Sys.Date()`"
...
region_count <- plastic_data_all %>%  
  group_by(region) %>% 
    count()

region_count

#Africa has the most coastal regions/territories
```
...
plastic_data_all %>%
  ...
  group_by(region) %>%
  summarise(total_mean = mean(total_mismanage_plastic))
  
#Africa has highest total, Oceania has lowest.
...
plastic_data_all %>% 
  mutate(pct_mismanaged_plastic_ocean = ((mismanaged_plastic_ocean * population) / (mismanaged_plastic * population)) * 100) %>%
  group_by(region) %>%
  summarise(median = median(pct_mismanaged_plastic_ocean, na.rm = T))
...
plastic_data_all %>%
  rename(
    pop = population,
    reg = region,
    mp = mismanaged_plastic,
    mpo = mismanaged_plastic_ocean
  )

plastic_data_all$region <- str_replace_all(plastic_data_all$region, "Latin America and The Caribbean", "LAC" )
